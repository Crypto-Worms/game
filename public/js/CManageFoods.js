function CManageFoods(oParentContainer) {
  ...
  this._spawnRandomFoods = function (oSection) {
    ...
    while (bCheckCol) {
      iXRand =
        Math.random() *
          (oSection.getRect().x +
            oSection.getRect().width -
            iOffsetX -
            (oSection.getRect().x + iOffsetX)) +
        oSection.getRect().x +
        iOffsetX;
      iYRand =
        Math.random() *
          (oSection.getRect().y +
            oSection.getRect().height -
            iOffsetY -
  ...
  this.checkCollisionFoodToObject = function (oObj, iRadius, iXRand, iYRand) {
    if (
      s_oGame.circleToCircleCollision(
        { x: iXRand, y: iYRand },
        oObj.getPos(),
        iRadius,
        oObj.getDim().w
  ...
  this.updateVisibility = function () {
    for (var i = 0; i < _aFoods.length; i++) {
      if (!_aFoods[i].getEaten()) {
        ...
        if (
          oPos.x > -_aFoods[i].getDim().w + s_iOffsetX &&
          oPos.x < CANVAS_WIDTH + _aFoods[i].getDim().w - s_iOffsetX &&
          oPos.y > -_aFoods[i].getDim().h + s_iOffsetY &&
          oPos.y < CANVAS_HEIGHT + _aFoods[i].getDim().h - s_iOffsetY
        ) {
          _aFoods[i].setVisible(true);
        } else {
