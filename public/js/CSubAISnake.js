function CSubAISnake(oSnake, iTimeFollow) {
  ...
  this._init = function () {
    _fTimeTurn =
      Math.random() * (AI_WAIT_TIME_FOR_CHANGE_DIR.max - AI_WAIT_TIME_FOR_CHANGE_DIR.min) +
      AI_WAIT_TIME_FOR_CHANGE_DIR.min;
  };
    ...
    if (_fTimeChangeDir < 0) {
      if (_fTimeTurn > 0) {
        ...
        _fTimeTurn =
          ...
          AI_WAIT_TIME_FOR_CHANGE_DIR.min;
        _fTimeChangeDir =
          Math.random() * (AI_TIME_CHANGE_DIR.max - AI_TIME_CHANGE_DIR.min) +
          AI_TIME_CHANGE_DIR.min;
      }
    } else {
