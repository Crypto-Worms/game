function CSnake(
  ...
  this._init = function (iX, iY, iType, iStartQueueLenght, oSprite) {
    ...
    _oOpenMounthDim = {
      w: _oCollision.w * OPEN_MOUNTH_DISTANCE_RATE,
      h: _oCollision.h * OPEN_MOUNTH_DISTANCE_RATE,
    };

    for (var i = 0; i < iStartQueueLenght; i++) {
      this.createAQueque(
        _oSnake.x,
        _oSnake.y + DISTANCE_SINGLE_QUEUE * (_aQueue.length + 1),
        false
      );
    }
  ...
  this.createCollision = function () {
    _oRectangleMeasure = {
      x: _oSnake.x - _iRadius,
      y: _oSnake.y - _iRadius,
      w: _iDiameter,
      h: _iDiameter,
    };

    _oRectangle = new createjs.Rectangle(
      _oRectangleMeasure.x,
      _oRectangleMeasure.y,
      _oRectangleMeasure.w,
      _oRectangleMeasure.h
    );
