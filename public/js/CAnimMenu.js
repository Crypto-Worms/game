function CAnimMenu(oParentContainer) {
  ...
  this._init = function () {
    ...
    _oGoodSnake = new CSnake(
      -100,
      CANVAS_HEIGHT_HALF,
      s_oSpriteLibrary.getSprite('snake_head_' + iTypeGood),
      iTypeGood,
      iLengthGoodQueue,
      0,
      _oContainer
    ...
    _fStopGoodMovement =
      s_oSpriteLibrary.getSprite('snake_parts_' + iTypeGood).height *
        (iLengthGoodQueue / (MENU_SNAKES_VELOCITY / 20)) +
      CANVAS_WIDTH -
      s_iOffsetX;

    var iTypeBad = Math.floor(Math.random() * 3);
    var iLengthBadQueue = 25;
    _oBadSnake = new CSnake(
      -100,
      CANVAS_HEIGHT_HALF,
      s_oSpriteLibrary.getSprite('snake_head_' + iTypeBad),
      iTypeBad,
      iLengthBadQueue,
      0,
      _oContainer
    ...
    _oLogo = new CLogo(
      CANVAS_WIDTH_HALF,
      s_oSpriteLibrary.getSprite('logo').height * -0.5,
      _oParentContainer
    );
    ...
    _fStopBadMovement =
      s_oSpriteLibrary.getSprite('snake_parts_' + iTypeBad).height *
        (iLengthBadQueue / (MENU_SNAKES_VELOCITY / 20)) +
      CANVAS_WIDTH -
      s_iOffsetX;
  };
