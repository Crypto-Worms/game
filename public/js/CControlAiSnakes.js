function CControlAiSnakes() {
  ...
  this.fieldOfViewFood = function (oSnake, oSubAI, oLines) {
    ...
    var oLinePos = new CVector2(
      oSnake.getX() + DISTANCE_AI_DETECT_FOOD * vDirPos.getX() - oSnake.getX(),
      oSnake.getY() + DISTANCE_AI_DETECT_FOOD * vDirPos.getY() - oSnake.getY()
    );
    var oLineNeg = new CVector2(
      oSnake.getX() + (DISTANCE_AI_DETECT_FOOD * vDirNeg.getX() - oSnake.getX()),
    ...
    if (SHOW_FIELD_OF_VIEW) {
      var oGraphPos = new CVector2(
        oSnake.getX() - DISTANCE_AI_DETECT_FOOD * vDirPos.getX(),
        oSnake.getY() - DISTANCE_AI_DETECT_FOOD * vDirPos.getY()
      );
      var oGraphNeg = new CVector2(
        oSnake.getX() - DISTANCE_AI_DETECT_FOOD * vDirNeg.getX(),
    ...
    if (!oSubAI.ignorePlayer()) {
      if (!this.countFollowersAI() || oSnake.getTarget().target === AI_PLAYER) {
        ...
        if (
          ...
          fAngleNeg < fAngle &&
          fMagLinePos >
            vCast.length2() /*&& oSnake.getLengthQueue() > oPlayerSnake.getLengthQueue()*/
        ) {
