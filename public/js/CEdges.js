function CEdges(oParentContainer) {
  ...
  this._init = function () {
    ...
    var iXLastEdge =
      iX + oSpriteEdgeLR.height * EDGES_PROPERTIES.w - oSpriteEdgeLR.width;
    SPAWN_FOODS_RANGE.xMax = iXLastEdge - EDGEBOARD_X * 0.5;
    EDGES_PROPERTIES.xMax = iXLastEdge;

    for (var i = 0; i < EDGES_PROPERTIES.h; i++) {
      _aEdges.push(
        new CEdge(iX, iY, { lr: true, val: 1 }, oSpriteEdgeLR, _oContainer)
      );
      _aEdges.push(
        new CEdge(
          iXLastEdge,
  ...
  this.createEdgesCollision = function (iLastX, iLastY, oSprite) {
    var aPoints = [
      {
        x: EDGES_PROPERTIES.x,
        y: EDGES_PROPERTIES.y,
        w: iLastX + oSprite.height * 0.5,
        h: oSprite.width - 17,
        normal: new CVector2(0, 1),
      }, //UP
      {
        x: EDGES_PROPERTIES.x,
        y: EDGES_PROPERTIES.y + oSprite.width,
        w: oSprite.width - 20,
        h: iLastY + oSprite.height * 0.5,
        normal: new CVector2(1, 0),
      }, //LEFT
      {
        x: EDGES_PROPERTIES.x,
        y: iLastY,
    ...
    for (var i = 0; i < aPoints.length; i++) {
      ...
      if (SHOW_COLLISION_SHAPE) {
        ...
        oShape.graphics
          .beginFill('#00ff00')
          .drawRect(aPoints[i].x, aPoints[i].y, aPoints[i].w, aPoints[i].h);
        oShape.alpha = 0.5;
